...
st.markdown("Erfassung von Mängeln und Schäden mit Bild-Upload, Unterschrift und automatisierter PDF-Generierung.")
...
with st.form("schaden_form"):
    ...
    st.header("5. Bestätigung & Digitale Signatur")
    col_sig1, col_sig2 = st.columns(2)
    with col_sig1:
        unterschrift_mieter = st.text_input("Name (für digitale Signatur Mieter / Anwesender)", placeholder="Vor- und Nachname eingeben")
    with col_sig2:
        unterschrift_kare = st.text_input("Name (für digitale Signatur KARE-Immobilien)", value=bearbeiter)
    
    protokoll_bestätigt = st.checkbox("Hiermit wird die Richtigkeit des Protokolls und der erfassten Mängel bestätigt.")

    submit_button = st.form_submit_button(label="Schadensprotokoll als PDF generieren")

if submit_button:
    if not protokoll_bestätigt:
        st.error("Bitte bestätigen Sie das Protokoll über die Checkbox, bevor Sie das PDF generieren.")
    else:
        images_html = ""
        if uploaded_files:
            ...
            <div class="signature-section">
                ...
                <div style="width: 100%;">
                    <div class="sig-box" style="float: left;">
                        <b>{unterschrift_mieter if unterschrift_mieter else '____________________'}</b><br>
                        Unterschrift Mieter / Anwesender
                    </div>
                    <div class="sig-box" style="float: right;">
                        <b>{unterschrift_kare if unterschrift_kare else '____________________'}</b><br>
                        Unterschrift KARE-Immobilien
                    </div>
                </div>
